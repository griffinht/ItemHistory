...
public class ItemCmd implements CommandExecutor, TabCompleter {
        ...
        if (args[0].equalsIgnoreCase("add")) {
            ...
            if (args.length == 2) {
                ...
                if (nodes.size() == 0) sender.sendMessage("Could not find trackedItem with name " + args[1]);
                    ...
                    for (Node<TrackedItem> node : nodes)
                        for (Node<TrackedItem> childNode : node.getChildren())
                            sender.sendMessage(childNode.getData().getEventName());
                }
                ...
                if (nodes.size() == 0) sender.sendMessage("Could not find trackedItem with name " + args[1]);
                    ...
                    sender.sendMessage("Children of " + args[1]);
                    for (Node<TrackedItem> node : nodes)
                        for (Node<TrackedItem> child : node.getChildren(Integer.parseInt(args[2])))
                            sender.sendMessage(child.getData().getEventName());
                }
            } else sender.sendMessage("Not enough arguments");
            return true;
            ...
            if (args.length == 2) {
                ...
                if (nodes.size() == 0) sender.sendMessage("Could not find trackedItem with name " + args[1]);
                    ...
                    for (Node<TrackedItem> node : nodes)
                        for (Node<TrackedItem> child : node.getChildren())
                            sender.sendMessage(child.getData().getEventName());//todo parents
                }
            } else if (args.length == 3) {
                List<Node<TrackedItem>> nodes = plugin.trackedItemTree.getByName(args[1]);
                if (nodes.size() == 0) sender.sendMessage("Could not find trackedItem with name " + args[1]);
            ...
            for (Node<TrackedItem> node : plugin.trackedItemTree.getRoots())
                sender.sendMessage(node.getData().getEventName());
            return true;
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        if (args.length == 1) {
            for (String string : new String[]{"add", "get", "getChildren", "getParents", "list", "addAsChild", "roots", "leaves", "go", "map"})
                if (checkCompletions(string, args[0])) completions.add(string);
            return completions;
        } else if (args.length == 2) {
            if (args[0].equalsIgnoreCase("add")) return completions;
            else if (args[0].equalsIgnoreCase("get") || args[0].equalsIgnoreCase("getChildren") || args[0].equalsIgnoreCase("getParents") || args[0].equalsIgnoreCase("addAsChild"))
                for (Node<TrackedItem> node : plugin.trackedItemTree.getAllNodes())
                    if (checkCompletions(node.getData().getEventName(), args[1]))
                        completions.add(node.getData().getEventName());//long boi
            return completions;
        } else if (args.length == 3) {
